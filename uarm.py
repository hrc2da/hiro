...
class UArmDelegate(DefaultDelegate):
    def __init__(self,buffer_length=0):
        DefaultDelegate.__init__(self)
        self.messages = Queue(maxsize=buffer_length) # Do we really need a queue for this??
    ...
    def read(self):
        if not self.read_flag or self.messages.empty():
            return None
        try:
            ...
            raise(e) # this is a useless try/catch
...
class UArm(object):
    def __init__(self, port_name=None, mac_address=None, timeout=2, debug=False, logger=None, ble=False):
        """
        ...
        self.ble_timeout = self.timeout = timeout
        self.ble = ble
    ...
    def connect(self):
        ...
        if self.ble == False:
            ...
            if self.mac_address is None:
                raise(ValueError("Scanning for peripherals is not yet implemented, please specify a mac address. --Matt."))
            try:
                print("Connecting to BLE")
                self.ble_delegate = ble_delegate = UArmDelegate()
                self.peripheral = peripheral = Peripheral(self.mac_address).withDelegate(ble_delegate)
                tries = 0
                while self.peripheral.getState() != 'conn':
                    self.peripheral.connect()
                    tries += 1
                    if tries > 5:
                        raise(f"Could not connect to {self.mac_address}") # and hit an exception below
                    
                self.services = services = list(peripheral.getServices())
                self.ble_channel = services[-1].getCharacteristics()[0] # hm-10 uses a custom characteristic on the third service to communicate
    ...
    def disconnect(self):
        ...
        if self.ble:
            ...
            print(f"Disconnect from {self.mac_address}")
        else:
    ...
    def send_and_receive(self, msg):
        ...
        if self.connection_state:
            ...
            if self.ble:
                print(_msg)
                for i in range(len(_msg)//20+1): #max-char-length of 20 in packet
                    start = i*20
                    status = self.ble_channel.write((_msg[start:start+20]).encode())
                line = ''
                while len(line)< 1 or line[-1] != '\n': # TODO: GO OVER THIS LOGIC PLEASE
                    if not self.peripheral.waitForNotifications(self.ble_timeout):
                        break
                    if self.ble_delegate.read_flag:
                        try:
                            response = self.ble_delegate.read()
                ...
                if line == '':
                    ...
                    return msg_id,self.ble_delegate.process_line(line)
            else:
        ...
        if self.connection_state:
            ...
            if self.ble:
                ...
                for i in range(len(_msg)//20+1): #max-char-length of 20 in packet
                    start = i*20
                    status = self.ble_channel.write((_msg[start:start+20]).encode())
                while self.peripheral.waitForNotifications(self.ble_timeout):
                    self.ble_delegate.read()# just clear the notifications
                # self.peripheral.waitForNotifications(self.ble_timeout)
            elif PY3:
