...
from wordnet.coding import encode
from wordnet.utils import compute_scale_down, prob_true
...
class DataLoaderIAM:
    ...
    def __init__(self, dataset, batch_size, input_size, output_size, device='cuda'):
    ...
    def __getitem__(self, item):
        ...
        for b in range(self.batch_size):
            ...
            batch_indices.append(shuffled_idx)
            ...
            if self.is_random:
                ...
                if prob_true(0.75):
                    ...
                    # img = cv2.warpAffine(img, M, dsize=self.input_size, dst=white_bg,
                    #                      borderMode=cv2.BORDER_TRANSPARENT)
        ...
        import pdb; pdb.set_trace()
        batch_imgs = torch.from_numpy(batch_imgs).to(self.torchdevice)
        batch_gt_maps = torch.from_numpy(batch_gt_maps.astype(np.float32)).to(self.torchdevice)
