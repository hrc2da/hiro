...
class DatasetHIRO:
    """loads the image and ground truth bounding box from HIRO json format"""

    def __init__(self, root_dir, input_size, output_size, caching=True, randomize=False, seed=42):
        ...
        labeldict = {}
        # load all the label jsons
        for fn_gt in sorted(gt_dir.files('*.json')):
            with open(fn_gt, 'r') as infile:
                newlabels = json.load(infile)
            labeldict = {**labeldict, **newlabels}


        img_files = [Path(k) for k in labeldict.keys()]
        if randomize:
            shuffle(img_files)
        for fn_img in img_files:
            full_fn_img = root_dir / fn_img
            if not full_fn_img.exists():
                continue
            self.fn_imgs.append(full_fn_img.abspath())
            self.fn_gts.append(labeldict[str(fn_img)])
            self.num_samples += 1

            if self.caching:
                img = cv2.imread(full_fn_img.abspath(), cv2.IMREAD_GRAYSCALE)
                img = cv2.resize(img, dsize=None, fx=self.loaded_img_scale, fy=self.loaded_img_scale)
                gt = self.parse_gt(self.fn_gts[-1])
    ...
    def parse_gt(self, fn_gt):
        #fn_gt here is the ACTUAL list of tuples of (word, top_left xy, bottom_right xy) for the image
        

        aabbs = []  # list of all axis aligned bounding boxes of current sample
        for box in fn_gt:
            word, top_left, bottom_right = box
            xmin,ymin = top_left
